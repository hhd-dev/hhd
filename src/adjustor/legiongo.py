...
def execute_acpi_command(command_parts):
    ...
    Args:
        command_parts (list): The parts of the ACPI command to be executed.
    
    Returns:
        str: The output from the ACPI command execution.
    """
    command = ['echo'] + command_parts + ['|', 'sudo', 'tee', '/proc/acpi/call;', 'sudo', 'cat', '/proc/acpi/call']
    try:
        result = subprocess.run(command, shell=False, check=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def set_tdp_value(mode, wattage):
    ...
    command_parts = [f"\\_SB.GZFD.WMAE 0 0x12 {{0x00, 0xFF, {mode_code}, 0x01, {wattage}, 0x00, 0x00, 0x00}}"]

...
class AdjustorPlugin(HHDPlugin):
    ...
    def ensure_type_key(self, settings_dict):
        if not isinstance(settings_dict, dict):
            logger.warning(f"Expected a dict in ensure_type_key, got {type(settings_dict)}")
            return
    ...
    def find_changed_keys(self, old_conf, new_conf, parent_key=''):
        if not isinstance(new_conf, dict) or not isinstance(new_conf, dict):
            logger.warning(f"Expected a dict in find_changed_keys, got {type(new_conf)}")
            return {}
        changed_keys = {}
        for section, settings in new_conf.items():
            ...
            if isinstance(settings, dict) and 'children' in settings:
                ...
                nested_changes = self.find_changed_keys(old_children, children, full_key)
                ...
                old_value = old_conf[section].get('value', None)  # Get the old value safely
    ...
    def update(self, conf: Config):
        ...
        for key, value in changed_keys.items():
            if key in ["tdp_management.tdp_fast", "tdp_management.tdp_slow", "tdp_management.tdp_steady"]:
    ...
    def handle_power_mode(self, mode):
        ...
        tdp_mode = settings.get('tdp_management', {}).get('children', {}).get('tdp_mode', {}).get('value', None)
        if tdp_mode == 'ryzenadj':
            ...
            lg.set_smart_fan_mode({'quiet': 1, 'balanced': 2, 'performance': 3, 'custom': 255}[mode])
            
        settings['tdp_management']['children']['tdp_mode']['children']['manufacturer']['legion_go']['powermode'] = {'value': mode}
        #Save the modified settings back to the YAML
        save_state_yaml(settings_path, {}, settings)


    def handle_tdp_change(self, key, value):
        tdp_mode = self.last_conf.get('tdp_management', {}).get('children', {}).get('tdp_mode', {}).get('value', None)

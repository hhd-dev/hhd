...
Layout recovered from native JIT disassembly of CoolingStatus.From / .Fill
in CompatLayerCT.exe (see disasm_cooling.txt, method 01186 / 01187).

GATT: service 0xFFE0, characteristic 0xFFE1 (read+write), 64 bytes.

IMPORTANT: The read (From) and write (Fill) directions use DIFFERENT byte
indices for the same logical fields. See the read and write tables below.
"""
...
# Chunked write protocol: Dock requires 3x20-byte frames with 0x1C/0x2C/0x3C headers.
...
class CoolingStatus:
    ...
    def to_write_bytes(self, current: bytes | bytearray) -> bytearray:
        ...
        out[4] = self.mode
        # Preserve read-only status bytes (5, 8).

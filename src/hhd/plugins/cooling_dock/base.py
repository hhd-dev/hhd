...
# Only load the cooling dock plugin on devices that have one.
SUPPORTED_PRODUCTS = ("ONEXPLAYER SUPER X", "ONEXPLAYER APEX")

# Scan backoff limits (seconds)
SCAN_BACKOFF_MIN = 5
SCAN_BACKOFF_MAX = 15
SCAN_BACKOFF_FACTOR = 2

# BLE watchdog: Max seconds without successful GATT read before sync loop breaks.
GATT_WATCHDOG_TIMEOUT = 20

# GATT operation timeout to prevent async hangs.
GATT_OP_TIMEOUT = 10

# Max consecutive GATT errors before forcing disconnect (anti-flap).
SYNC_RETRY_MAX = 3

# Delay (s) before reconnect to allow BlueZ cleanup.
RECONNECT_DELAY = 2

# Delay (s) between transient GATT error retries.
SYNC_RETRY_DELAY = 2

# Sync loop interval (s). >2s required to avoid firmware BLE exhaustion.
SYNC_READ_INTERVAL = 5


# Grace period (s) before dropping dock_running (and TDP) to ride out packet drops.
DOCK_RUNNING_GRACE = 10
...
class CoolingDockPlugin(HHDPlugin):
    ...
    def settings(self) -> HHDSettings:
        ...
        if not BLEAK_AVAILABLE:
            ...
            with self.conf_lock:
                ...
                # Dynamically hide controls if dock sync is disabled or no dock is selected
                if not self.enabled or not self._mac_address:
                    children = base["cooling_dock"]["dock"]["children"]
                    for key in ["mode", "fan_curve", "rgb"]:
                        if key in children:
                            del children[key]
    ...
    def _publish_dock_running(self, running: bool):
        # Publish running state for TDP adjustments (only on change).
        with self.conf_lock:
            conf = self.conf
    ...
    def _publish_status(self, status: str, fan_progress: dict | None):
        # Publish UI status (only on change).
        with self.conf_lock:
            conf = self.conf
    ...
    def _publish_disconnected_if_stale(self):
        # Delay dropping dock_running to ride out transient BLE packet drops.
        if (
    ...
    async def _write_state(self, client, state: bytearray):
        """Write a modified 64-byte state to the dock using the chunked
        protocol (3 x 20-byte frames with 0x1C/0x2C/0x3C headers).

        A single 64-byte write is silently ignored by the dock, so the state
        must be split into chunks. Retries handle transient "In Progress"
        errors from the dock while it is still processing a previous write.
        """
    ...
    async def _connect_and_sync(self):
        # Enforce grace period to ride out transient BLE drops.
        ...
        if not ble_device:
            ...
            # Exponential backoff: increase delay for next scan
        ...
        # Listen for immediate BlueZ link loss.
        ...
        # Retry transient connect errors (e.g. BlueZ "In Progress").
        ...
        now = time.time()
        # Log reconnect duration for grace period validation.
        ...
        # Save MAC address for sticky pairing if we don't have it yet
        ...
        # Register connected MAC for UI. Do NOT emit settings to avoid reload storm.
        with self.conf_lock:
        ...
        while (
            ...
            # BLE watchdog: if no successful GATT read for too long,
            # assume the dock disconnected (BLE supervision timeout gap).
            ...
            try:
                ...
                # Dynamic hardware detection (Air vs Water Cooled)
                ...
                # Only write on change. Writing every cycle hammers the dock and triggers GATT errors.
        ...
        # Do NOT drop dock_running here. _publish_disconnected_if_stale() handles the grace period.

        # Pause to let BlueZ clean up link.
    ...
    async def _bluez_start_discovery(self, target_mac: str | None, timeout: int = 10):
        ...
        try:
            ...
            # Poll for the device to appear so we can stop scanning early
    ...
    def _find_dock_in_bluez_objects(
        ...
        """Query BlueZ D-Bus directly for the dock.

        This finds devices even if they are bonded (which BleakScanner filters out)
        or not currently advertising but known to BlueZ.
        """
        try:
            import dbus
    ...
    async def _find_dock(self) -> BLEDevice | None:
        try:
            ...
            # If no dock is selected ("None") and user didn't request a scan, do nothing.
            if not target_mac and not scan_req:
                return None

            # 1. First, check if BlueZ already knows the device
            device = self._find_dock_in_bluez_objects(target_mac)
            if device:
                # Need to run discover to populate the dropdown
                if scan_req:
                    await self._populate_dropdown()
                return device

            # 2. If not found, trigger BlueZ discovery directly.
            # We poll during the scan and exit early if found, so 10s is safe.
            scan_timeout = 10
            ...
            # 3. Fallback to bleak scanner if D-Bus fails
            if target_mac:
    ...
    async def _populate_dropdown(self):
        """Populate the UI dropdown with discovered devices."""
        discovered = {"": "None"}
        found_macs = []
        ...
        with self.conf_lock:
            ...
            # If "None" was selected but we found a dock during a manual scan, auto-select it
    ...
    def _remove_stale_bond(self, mac: str):
        """Remove any existing BlueZ bond/pairing for the dock.

        The dock's HID profile causes BlueZ to trigger SMP pairing, which
        creates a bond. Because the dock uses a random address type, the
        bond keys quickly become stale. A stale bond prevents BleakScanner
        from seeing the device, and causes BlueZ to auto-connect with bad keys.
        Windows never pairs with the dock (WinRT stateless connection). We
        replicate that by deleting the bond before every connection attempt.
        """
        if not mac:
            return
        ...
        # Fallback: bluetoothctl (works without python-dbus)
        try:
            import subprocess
    ...
    def _forget_bluez_device(self):
        """Unpair and remove the dock from BlueZ.

        The dock is Trusted, so BlueZ auto-connects it and keeps it paired.
        "Forget" must remove it from BlueZ entirely, otherwise the dock
        stays connected and fan control keeps working.
        """
    ...
    def _find_connected_dock_via_bluez(
        ...
        """Find a CoolingSystem dock that is already connected to BlueZ.

        Connected BLE devices stop advertising, so bleak scans cannot see
        them. After a sync-loop break or reconnect, the dock is often still
        connected via BlueZ (auto-connect to a trusted device) while not
        advertising — this queries BlueZ directly to recover it.

        Returns a BLEDevice carrying the BlueZ D-Bus path. BleakClient
        uses ``details["path"]`` to attach to the existing connection
        without requiring a scan, which is essential for a connected but
        non-advertising dock.
        """
        try:
            import dbus
        ...
        # Fallback: bluetoothctl (assumes hci0).
        try:
            import subprocess
...
def _is_supported_device() -> bool:
    """Check if the current device is a supported OneXPlayer model."""
    try:
